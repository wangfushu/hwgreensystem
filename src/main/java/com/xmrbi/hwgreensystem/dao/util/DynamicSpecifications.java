...
public final class DynamicSpecifications {
    ...
    public static <T> Specification<T> bySearchFilter(final Collection<SearchFilter> filters, final Class<T> clazz) {
        return new Specification<T>() {
                    ...
                    for (SearchFilter filter : filters) {
                        ...
                        if (multiProperty) {
                            ...
                            if (filter.value instanceof List) {
                                ...
                                List<Predicate> orPredicateValues = Lists.newArrayList();
                                ArrayList<String> list = (ArrayList<String>) filter.value;
                                for (String value : list) {
                                    orPredicateValues
                                            .add(buildPredicate(fieldNames[0], value, filter.operator, root, builder));
                                }
                                Predicate[] predicateArrValue = new Predicate[orPredicateValues.size()];
                                predicates.add(builder.or(orPredicateValues.toArray(predicateArrValue)));
                            } else {
                                ...
                                for (String fieldName : fieldNames) {
                                    orPredicates
                                            .add(buildPredicate(fieldName, filter.value, filter.operator, root, builder));
                                }
                            ...
                            predicates.add(
                                    buildPredicate(filter.fieldName, filter.value, filter.operator, root, builder));
                        }
            ...
            private Predicate buildPredicate(String fieldName, Object value, SearchFilter.Operator operator,
                ...
                switch (operator) {
                    ...
                    case ILIKE:
                        ...
                        break;
                    case LIKEIGNORECASE:
                        predicate = builder.like(builder.lower(expression),
                                "%" + StringUtils.lowerCase(value.toString()) + "%");
                        break;
                    case TLIKE:
                        predicate = builder.like(expression, value + "%");
                        break;
